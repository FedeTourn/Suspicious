...
public class GoE extends SearchAction {
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s) {
        ...
        if (agentState.getVisitedPositions().contains(RobotAgentState.E)) {
            return null;
        }
        ArrayList<String> successors = new ArrayList<String>(agentState.getSuccessors());
        if (successors != null) {
            int index = successors.indexOf(RobotAgentState.E);
            if (index >= 0) {
                agentState.setPosition(RobotAgentState.E);
                return agentState;
            }
