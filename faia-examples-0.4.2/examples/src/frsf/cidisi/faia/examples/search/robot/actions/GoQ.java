...
public class GoQ extends SearchAction {
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s) {
        ...
        if (agentState.getVisitedPositions().contains(RobotAgentState.Q)) {
            return null;
        }

        ArrayList<String> successors = new ArrayList<String>(agentState.getSuccessors());
        if (successors != null) {
            int index = successors.indexOf(RobotAgentState.Q);
            if (index >= 0) {
                agentState.setPosition(RobotAgentState.Q);
                return agentState;
            }
