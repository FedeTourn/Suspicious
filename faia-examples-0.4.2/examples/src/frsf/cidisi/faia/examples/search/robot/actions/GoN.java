...
public class GoN extends SearchAction {
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s) {
        ...
        if (agentState.getVisitedPositions().contains(RobotAgentState.N)) {
            return null;
        }
        ArrayList<String> successors = new ArrayList<String>(agentState.getSuccessors());
        if (successors != null) {
            int index = successors.indexOf(RobotAgentState.N);
            if (index >= 0) {
                agentState.setPosition(RobotAgentState.N);
                return agentState;
            }
