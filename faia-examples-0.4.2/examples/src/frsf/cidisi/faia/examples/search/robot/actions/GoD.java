...
public class GoD extends SearchAction {
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s) {
        ...
        if (agentState.getVisitedPositions().contains(RobotAgentState.D)) {
            return null;
        }
        ArrayList<String> successors = new ArrayList<String>(agentState.getSuccessors());
        if (successors != null) {
            int index = successors.indexOf(RobotAgentState.D);
            if (index >= 0) {
                agentState.setPosition(RobotAgentState.D);
                return agentState;
            }
