...
pop(CPlan,Agenda,DIC0,FPlan,DB) :-
   select_mod(Goal,Agenda,Agenda1),
...
select_mod(goal(G,GG),[goal(G,GG)|A],A) :-
   primitive(G),!.
select_mod(goal((X \= Y),GG),[goal((X\=Y),GG)|A],A) :- !.
select_mod(P,[goal(G,GG)|R],A) :-
   (G <- B),
...
%%% modificado mayo-2005
%%%   incorporate_causal_link(cl(N0,P,A1),As,Os1,NOs),
   check_causal_links([cl(N0,P,A1)|Ls],As,Os1,NOs),
%%% fin modificado mayo-2005
...
solve_goal(goal(P,A1),
   ...
   DB>0,
   achieves(Act0,P),
%%% agregado mayo-2005 : no permitir que se instance nuevamente el operador init
   Act0 \== init,
%%% fin agregado mayo-2005
%   writeln_mod(['*** new action ',act(DB,Act0), ' to achieve ',P,' for ',A1]),
...
%%%   incorporate_causal_link(cl(DB,P,A1),As,Os3,NOs),
   check_causal_links([cl(DB,P,A1)|Ls],[act(DB,Act0)|As],Os2,NOs),
%%% fin modificado mayo-2005
...
%   \+ member(A0<A1,L1),    % ommitted because of the cut for efficiency.
   \+ member(A1<A0,L1),
%   writeln_mod([' ... adding constraint ',A0<A1]),
...
%%% agregado mayo-2005 : chequear todo!!!
check_causal_links([],_,Os,Os).
check_causal_links([CL|Ls],As,Os,NOs) :-
   check_cl(CL,As,Os,Os1),
   check_causal_links(Ls,As,Os1,NOs).

check_cl(_,[],Os,Os).
check_cl(CL,[A|RAs],Os,NOs) :-
   protect(CL,A,Os,Os1),
   check_cl(CL,RAs,Os1,NOs).
...
%%% incorporate_causal_link(CL, As, Os, NOs) :-
%%% %   writeln_mod(['Adding causal link: ',CL,nl,'  As=',As,nl,'  Os=',Os]),
%%%    incorporate_causal_link1(CL, As,Os,NOs)
%%% %,   writeln_mod([' Added causal link: ',CL,nl,'  As=',As,nl,'  Os=',NOs])
%%% .
...
%%% agregado mayo-2005 : no permitir que se produzcan amenazas si hay
%%%                      variables libres (no ground)
protect(cl(_,P,_),act(_,_),Os,Os) :-
   \+ ground(P), !.
protect(cl(_,P,_),act(_,A),Os,Os) :-
   \+ minimo_compromiso(A,P).
protect(cl(A0,P,A1),act(NA,A),Os,Os1) :-
   minimo_compromiso(A,P),
   enforce_order(A0,NA,A1,Os,Os1).

minimo_compromiso(A,P):- deletes(A,P1), ground(P1), P = P1.
%%% fin agregado mayo-2005
%%% lo que sigue fue quitado:
...
%,   writeln_mod(['Putting ',A,' after ',A1])
.
enforce_order(A0,A,_,Os,Os1) :-
   add_constraint(A<A0,Os,Os1)
%,   writeln_mod(['Putting ',A,' before ',A0])
.
...
%%%%  ya esté definido como primitiva
% append([],L,L).
% append([H|X],Y,[H|Z]) :-
%    append(X,Y,Z).
...
writeln_mod(L) :- \+ \+ (numbervars(L,0,_), writelnw(L) ).
writelnw([]) :- nl.
writelnw([nl|T]) :- !,nl, writeln_mod(T).
writelnw([H|T]) :- write(H), writeln_mod(T).
